...
word_emb_path = "glove.840B.300d.txt"  ## Not actually used in code
...
def get_args():
    ...
    parser.add_argument(
        "--data_path", type=str, default="datasets", help="path for storing the dataset"
    )
    ...
    parser.add_argument(
        "--dropout_v", type=float, default=0.1, help="dropout of visual LSTM out layer"
    )
    parser.add_argument(
        "--dropout_prj", type=float, default=0.1, help="dropout of projection layer"
    )

    # Architecture
    parser.add_argument(
        "--multiseed", action="store_true", help="training using multiple seed"
    )
    parser.add_argument(
        "--contrast", action="store_true", help="using contrast learning"
    )
    ...
    parser.add_argument(
        "--d_vh", type=int, default=16, help="hidden size in visual rnn"
    )
    parser.add_argument(
        "--d_ah", type=int, default=16, help="hidden size in acoustic rnn"
    )
    parser.add_argument(
        "--d_vout", type=int, default=16, help="output size in visual rnn"
    )
    parser.add_argument(
        "--d_aout", type=int, default=16, help="output size in acoustic rnn"
    )
    parser.add_argument(
        "--bidirectional", action="store_true", help="Whether to use bidirectional rnn"
    )
    parser.add_argument(
        "--d_prjh", type=int, default=128, help="hidden size in projection network"
    )
    parser.add_argument(
        "--pretrain_emb",
        type=int,
    ...
    parser.add_argument(
        "--beta", type=float, default=0.1, help="weight for lld item (default: 0.1)"
    )
    ...
    parser.add_argument(
        "--update_batch", type=int, default=1, help="update batch interval"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="language",
        help="one of {language, video, audio}",
    )
    ...
    parser.add_argument(
        "--is_test", action="store_true", help="Distinguish between eval and test"
    )
    #######################################################################################################
    parser.add_argument(
        "--save_model_to",
    ...
    parser.add_argument("--baseline_model_path", required=False, default=None, type=str)

    parser.add_argument("--audio_encoder_path", default=None, required=False)
    parser.add_argument("--video_encoder_path", default=None, required=False)

    parser.add_argument("--cmam_path", required=False)
