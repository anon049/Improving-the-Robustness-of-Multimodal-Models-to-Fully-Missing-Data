    ...
    def __init__(self, config):
        ...
        try:
            ...
            if True:
                v = np.concatenate(
                    (
                        train_split_noalign["vision"],
                        dev_split_noalign["vision"],
                        test_split_noalign["vision"],
                    ),
                    axis=0,
                )
                vlens = get_length(v)
                ...
                a = np.concatenate(
                    (
                        train_split_noalign["audio"],
                        dev_split_noalign["audio"],
                        test_split_noalign["audio"],
                    ),
                    axis=0,
                )
                alens = get_length(a)
                ...
                label = np.concatenate(
                    (
                        train_split_noalign["labels"],
                        dev_split_noalign["labels"],
                        test_split_noalign["labels"],
                    ),
                    axis=0,
                )
            ...
            all_id = np.concatenate(
                (
                    train_split_noalign["id"],
                    dev_split_noalign["id"],
                    test_split_noalign["id"],
                ),
                axis=0,
            )[:, 0]
            ...
            for i, idd in enumerate(all_id_list):
                ...
                if i < dev_start:
                    train.append(
                        (
                            (words, visual, acoustic, actual_words, _vlen, _alen),
                            _label,
                            idd,
                        )
                    )
