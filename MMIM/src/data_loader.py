...
# bert_tokenizer = BertTokenizer.from_pretrained('bert-large-cased', do_lower_case=True)
...
def get_loader(hp, config, shuffle=True):
    ...
    def collate_fn(batch):
        ...
        # Rewrite this
        def pad_sequence(
            sequences, target_len=-1, batch_first=False, padding_value=0.0
        ):
            if target_len < 0:
        ...
        sentences = pad_sequence(
            [torch.LongTensor(sample[0][0]) for sample in batch], padding_value=PAD
        )
        visual = pad_sequence(
            [torch.FloatTensor(sample[0][1]) for sample in batch],
            target_len=vlens.max().item(),
        )
        acoustic = pad_sequence(
            [torch.FloatTensor(sample[0][2]) for sample in batch],
            target_len=alens.max().item(),
        )
