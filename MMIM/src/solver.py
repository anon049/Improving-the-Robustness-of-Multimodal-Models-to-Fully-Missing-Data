    ...
    def __init__(
        ...
        if self.is_train:
            ...
            if self.is_train:
                ...
                for name, p in self.model.named_parameters():
                    ...
                    for p in mmilb_param + main_param:
                        if (
                            p.dim() > 1
                        ):  # only tensor with no less than 2 dimensions are possible to calculate fan_in/fan_out
            ...
            optimizer_main_group = [
                ...
                {
                    "params": main_param,
                    "weight_decay": hp.weight_decay_main,
                    "lr": hp.lr_main,
                },
            ]
            ...
            self.scheduler_mmilb = ReduceLROnPlateau(
                self.optimizer_mmilb,
                mode="min",
                patience=hp.when,
                factor=0.5,
                verbose=True,
    ...
    def train(self, epoch, mem_size, model, optimizer, criterion, stage=1):
        ...
        for i_batch, batch_data in enumerate(self.train_loader):
            ...
            if stage == 0:
                ...
                mem = {
                    "tv": {"pos": mem_pos_tv, "neg": mem_neg_tv},
                    "ta": {"pos": mem_pos_ta, "neg": mem_neg_ta},
                    "va": (
                        {"pos": mem_pos_va, "neg": mem_neg_va}
                        if self.hp.add_va
                        else None
                    ),
                }
            ...
            if i_batch % self.hp.log_interval == 0 and i_batch > 0:
                ...
                print(
                    "Epoch {:2d} | Batch {:3d}/{:3d} | Time/Batch(ms) {:5.2f} | Train Loss ({}) {:5.4f} | NCE {:.3f} | BA {:.4f}".format(
                        epoch,
                        i_batch,
                        num_batches,
    ...
    def train_and_eval(self):
        ...
        for epoch in range(1, self.hp.num_epochs + 1):
            ...
            if self.hp.contrast:
                train_loss = self.train(
                    epoch, mem_size, model, optimizer_mmilb, criterion, 0
                )

            # minimize all losses left
            train_loss = self.train(
                epoch, mem_size, model, optimizer_main, criterion, 1
            )
        ...
        if self.hp.dataset in ["mosei_senti", "mosei"]:
            ...
            accuracy = calc_metrics(
                y_true, y_pred, "test", True, self.hp.save_results_to
            )
            ##########################################################
        elif self.hp.dataset == "mosi":
